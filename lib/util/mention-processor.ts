...
export const formatMentionInput = ({
  ...
  labels,
  lists,
}: FormatMentionInputProps): string => {
  ...
  const formattedLabels = selectedLabels
    .map((label) => `@[${label.name}](hash:${label.id})`)
    .join(' ');
  const formattedList = selectedList
    ? `@[${selectedList.name}](at:${listId})`
    : '';
...
export const extractName = (input: string): string =>
  input.replace(listRegex, '').replace(labelRegex, '').trim();
...
export const extractListId = (input: string): string | undefined => {
  ...
  return listMatches
    ? listMatches[listMatches.length - 1].split(':')[1].slice(0, -1)
    : undefined;
};
...
export const extractLabelIds = (input: string): string[] | undefined => {
  ...
  const extractedLabelIds = labelMatches
    ? Array.from(
        new Set(labelMatches.map((match) => match.split(':')[1].slice(0, -1))),
      )
    : [];
